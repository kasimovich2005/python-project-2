...
class CommunicationETL:
    ...
    def transform_data(self, parsed_data: List[Dict], input_file: str) -> None:
        ...
        for item in parsed_data:
            ...
            # Dimension ID larni olish yoki yaratish
            ...
            # Meeting attendees dan email larni ajratish
            attendees = []
            ...
            all_emails = set()
            all_emails.update(speakers)
            all_emails.update(participants)
            all_emails.update(attendees)
            if host_email:
                all_emails.add(host_email)
            if organizer_email:
                all_emails.add(organizer_email)
            
            # User ID larni yaratish
            for email in all_emails:
                self.get_or_create_user_id(email)
            
            # Fact communication yozuvi - примерОтвета format
            comm_id = str(uuid.uuid4())
            
            # raw_data.xlsx dan original ma'lumotlar
            ...
            # Bridge table yozuvlari - примерОтвета format
            for email in all_emails:
...
if __name__ == "__main__":
    ...
    # Fayl yo'llarini belgilang - cross-platform uchun
    import os
    
    # Hozirgi papkada fayllarni qidirish
    current_dir = os.path.dirname(os.path.abspath(__file__))
    input_file = os.path.join(current_dir, r"E:\MAAB\python\project\raw_data.csv")
    output_file = os.path.join(current_dir, r"E:\MAAB\python\project\final.xlsx")
